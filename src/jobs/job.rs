...
use crate::{
    app::{
        events::{models::model::AddEventDto, services::service::{end_event, process_schedule_days}},
...
pub async fn stop_non_recurring_event(state: &web::Data<AppState>) -> Result<(), Box<dyn std::error::Error>> {
    let today = chrono::Utc::now().naive_utc().date();
